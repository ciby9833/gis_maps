...
class AnchorPoint {
  ...
  setControlHandles(handle1, handle2) {
    // 验证控制柄的有效性
    if (handle1 && (isNaN(handle1.lat) || isNaN(handle1.lng) || !isFinite(handle1.lat) || !isFinite(handle1.lng))) {
      console.warn("Invalid handle1 provided to setControlHandles:", handle1);
      handle1 = null;
    }

    if (handle2 && (isNaN(handle2.lat) || isNaN(handle2.lng) || !isFinite(handle2.lat) || !isFinite(handle2.lng))) {
...
class PathManager {
  ...
  createAnchorMarker(anchor, index) {
    ...
    try {
      ...
      marker.on("drag", (e) => {
        ...
        // 验证拖动位置的有效性
        if (isNaN(newPos.lat) || isNaN(newPos.lng) || !isFinite(newPos.lat) || !isFinite(newPos.lng)) {
  ...
  createControlHandleMarker(anchor, handleType, position, anchorIndex) {
    // 验证位置的有效性
    if (!position || isNaN(position.lat) || isNaN(position.lng) || !isFinite(position.lat) || !isFinite(position.lng)) {
  ...
  updateControlHandleLine(anchor, handleType) {
    ...
    if (line && handlePos) {
      // 验证锚点和控制柄位置的有效性
      if (isNaN(anchor.position.lat) || isNaN(anchor.position.lng) || isNaN(handlePos.lat) || isNaN(handlePos.lng)) {
  ...
  generateBezierPath() {
    ...
    for (let i = 0; i < this.anchors.length - 1; i++) {
      ...
      // 验证锚点位置
      if (isNaN(current.position.lat) || isNaN(current.position.lng) || isNaN(next.position.lat) || isNaN(next.position.lng)) {
  ...
  calculateBezierCurve(p0, p1, p2, p3, segments = 20) {
    ...
    if (!validatePoint(p0, "p0") || !validatePoint(p1, "p1") || !validatePoint(p2, "p2") || !validatePoint(p3, "p3")) {
      // 如果有无效点，返回直线
  ...
  updatePath() {
    ...
    try {
      ...
      if (pathPoints.length > 0) {
        // 验证路径点的有效性
        const validPoints = pathPoints.filter((point) => !isNaN(point.lat) && !isNaN(point.lng) && isFinite(point.lat) && isFinite(point.lng));
  ...
  generateGeometry() {
    ...
    try {
      ...
      // 验证路径点并转换为坐标
      const coordinates = pathPoints.filter((point) => !isNaN(point.lat) && !isNaN(point.lng) && isFinite(point.lat) && isFinite(point.lng)).map((point) => [point.lng, point.lat]);
